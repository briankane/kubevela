...
| `featureGates.enableSourceExpressions`                       | read $( ) property expressions, and with them SourceDefinition. Off by default because $(VAR_NAME) is Kubernetes' own env-var syntax and would be read as an expression (Alpha)                                                  | `false` |
| `featureGates.requireSourceExpressionOptIn`                  | while enableSourceExpressions is on, read expressions only in Applications annotated app.oam.dev/source-expressions=true, so enabling the feature is not a fleet-wide change (Alpha)                                             | `true`  |
