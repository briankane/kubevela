...
// With no live client the cached one is used unwrapped, which is what a test
// supplying a fake wants and what a manager that could not build an uncached
// client falls back to.
func TestTheCachedClientIsUsedAloneWithoutALiveOne(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = v1beta1.AddToScheme(scheme)
	...
	require.Equal(t, cached, h.definitionReader())
}
