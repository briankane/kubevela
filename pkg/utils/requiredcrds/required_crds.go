...
type CRD struct {
	...
	// Why records what depends on it, for required CRDs, or why its absence is
	// tolerable, for optional ones.
	Why string
}
...
func core(plural, kind, version, why string) CRD {
	return CRD{
		Plural:           plural,
...
// Required are the CRDs vela-core cannot run without.
//
// Every entry here is either watched by a controller or read on the reconcile
// path, so its absence is not a missing feature but a broken install.
var Required = []CRD{
	core("applications", "Application", "v1beta1", "watched by the Application controller"),
	core("applicationrevisions", "ApplicationRevision", "v1beta1", "every Application mints one"),
	core("resourcetrackers", "ResourceTracker", "v1beta1", "tracks dispatched resources; without it nothing can be garbage collected"),
	core("componentdefinitions", "ComponentDefinition", "v1beta1", "watched by the ComponentDefinition controller"),
	core("traitdefinitions", "TraitDefinition", "v1beta1", "watched by the TraitDefinition controller"),
	core("policydefinitions", "PolicyDefinition", "v1beta1", "watched by the PolicyDefinition controller"),
	core("workflowstepdefinitions", "WorkflowStepDefinition", "v1beta1", "watched by the WorkflowStepDefinition controller"),
	core("sourcedefinitions", "SourceDefinition", "v1beta1", "watched by the SourceDefinition controller"),
	core("definitionrevisions", "DefinitionRevision", "v1beta1", "every definition mints one"),
	core("workloaddefinitions", "WorkloadDefinition", "v1beta1", "read by the ComponentDefinition mutating webhook"),
	core("policies", "Policy", "v1alpha1", "read when a workflow step depends on a policy"),
	core("workflows", "Workflow", "v1alpha1", "read when an Application refers to an external workflow"),
}

// Optional are CRDs the chart ships whose absence vela-core survives.
//
// Listed rather than omitted so that adding a CRD to the chart forces a decision
// about it. TestEveryChartCRDIsClassified fails on anything that appears in
// neither list.
var Optional = []CRD{
	{
		Plural:           "packages",
		GroupVersionKind: schema.GroupVersionKind{Group: "cue.oam.dev", Version: "v1alpha1", Kind: "Package"},
		Why: "cuex external packages are loaded behind a flag and both call sites log " +
			"and continue on failure, so a missing CRD costs the feature and nothing else",
	},
}
