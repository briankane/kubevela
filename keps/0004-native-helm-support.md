...
- KubeVela v1.12: Alpha implementation (depends on KEP-0005, internal use only)
- KubeVela v1.13: Beta implementation (early access users)
...
- KEP-0005: Resource Dispatch Ordering (prerequisite for reliable CRD + CR deployment)

  ↳ KEP-0001: Umbrella Chart Architecture (depends on this KEP)
...
**Dependency on KEP-0005**: This KEP depends on KEP-0005 (Resource Dispatch Ordering) to handle the ordering of CRDs before Custom Resources. Without KEP-0005, Helm charts that include both CRDs and CRs would experience the same retry/failure patterns that currently affect other KubeVela components.

The Helm provider will render charts to standard Kubernetes resources and rely on the enhanced dispatch system from KEP-0005 to:
- Apply CRDs first and wait for establishment
- Apply Custom Resources after CRDs are ready
- Maintain the reliability users expect from native Helm

This approach keeps the Helm provider focused purely on chart rendering while leveraging KubeVela's improved infrastructure for reliable resource deployment.
